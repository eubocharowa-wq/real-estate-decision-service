...
| GOV-01 | ЕИСЖС — `наш.дом.рф` | Официальный источник | Проектные декларации по 214-ФЗ: адрес, кадастровый номер, площадь, этажность, срок ввода (квартал). Цены, доступности и финансирования не содержит | Ручной ввод. Карточки объектов адресуются query-параметрами, автоматический сбор не согласован и адаптера нет | `PUBLIC_OFFICIAL_SOURCE` | **P1 источник реального пилотного набора через `manual_curated`** |
...
# 13b. GOV-01 — ЕИСЖС (наш.дом.рф)

ЕИСЖС — единая информационная система жилищного строительства. Застройщики
публикуют там проектные декларации в силу 214-ФЗ, поэтому правовая природа
данных чистая: это обязательная публикация, а не коммерческий контент.

Домен `наш.дом.рф` делится с FIN-01: на нём же лежат материалы по программам
ДОМ.РФ. Разделение зафиксировано явным `shared_ownership_rule` в реестре —
GOV-01 отвечает за декларации об объектах, FIN-01 за правила программ.
...
- площадь;
- этажность;
- срок ввода в эксплуатацию с точностью до квартала.

Чего не даёт: цену конкретной квартиры, доступность, любые условия
финансирования, время в пути, отделку, балкон. Декларация описывает объект, а
не сделку.

### Статус

```text
Technical: MANUAL_ONLY — карточки объектов адресуются query-параметрами,
           collection scope их отклоняет; адаптер не написан и не согласован
Role: authoritative object-declaration source
Legal/access: PUBLIC_OFFICIAL_SOURCE, публикация обязательна по 214-ФЗ
```

### Решение

Источник реального пилотного набора по Туле через ручной ввод (`manual_curated`).
Автоматический сбор в реестре запрещён (`automation: denied`, `collect: none`).
Сроки ввода фиксируются как обязательство застройщика (`claimed`), статические
характеристики объекта — как факт декларации (`confirmed`).
