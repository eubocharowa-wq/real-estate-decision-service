...
operational metadata. Provisional TTL values are policy configuration, not
immutable product constants. `valid_until_overrides` marks fields where an
explicit validity date takes precedence over the generic TTL.
...
3. Define environment approvals, all production gates, field authority,
   freshness, attribution, health, and operational metadata.
