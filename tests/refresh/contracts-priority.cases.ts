...
describe("refresh contracts and priority policy", () => {
  it("formalizes a versioned RefreshTask with the complete reason taxonomy", () => {
    const queue = new InMemoryRefreshQueueRepository();
    const task = queue.enqueue(makeRefreshRequest()).task;
  ...
  it("uses a deterministic execution idempotency boundary per attempt", () => {
    const queue = new InMemoryRefreshQueueRepository();
    const task = queue.enqueue(makeRefreshRequest()).task;
